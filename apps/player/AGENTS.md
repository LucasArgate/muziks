...
**Playback no browser (implementado):** fluxo SDK + poll API + merge + fila «Próximas no Spotify» + debug — [`docs/tech/PLAYBACK-MASTER-CLIENT-SYNC.md`](../../docs/tech/PLAYBACK-MASTER-CLIENT-SYNC.md). **Estado UI (Zustand SSOT):** `@muziks/playback-client` (`useMasterPlaybackStore`, `useSpotifyQueueStore`, `useMuziksQueueStore`). ADRs: [ADR-playback-hybrid-realtime.md](../../docs/tech/ADR-playback-hybrid-realtime.md), [ADR-spotify-state-sync.md](../../docs/tech/ADR-spotify-state-sync.md).
