...
function reportBroadcastError(error: unknown): void {
  if (process.env.NODE_ENV !== "production") {
    console.error("[muziks:realtime] queue snapshot broadcast failed", error);
  }
}
...
export async function broadcastQueueSnapshotFromServer(
  ...
  if (!parsed.success) {
    reportBroadcastError(parsed.error);
    return;
  }
