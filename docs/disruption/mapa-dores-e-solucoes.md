...
| N participantes no mesmo salão repetem leitura da mesma fila no servidor; custo de egress e latência percebida em rajada | Campo / Engenharia | Rajadas e ICP em [05-insights](../analytics/reports/05-insights-para-muziks-hoje.md); polling PoC em [STACK §3](../tech/STACK-E-FASES-DE-MIGRACAO.md) | No **mesmo Hub físico**, estado de **leitura** pode propagar-se localmente; **votos** permanecem no servidor | Experimento **WebRTC DataChannel**: âncora no **telão** (prioridade) ou **mesh** parcial entre peers; snapshot assinado; fallback HTTP | [hub-local-webrtc-e-fanout.md](./hub-local-webrtc-e-fanout.md) |
...
- **Hub local WebRTC (fan-out da fila)** — hipótese em [hub-local-webrtc-e-fanout.md](./hub-local-webrtc-e-fanout.md); spikes E1/E2 e decisão E3 **pendentes**; não altera polling do MVP até conclusão do experimento.
