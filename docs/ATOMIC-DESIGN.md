...
2. **Moléculas** — **combinação de poucos átomos** com um propósito único: campo de busca (input + botão), linha de meta-dado (ícone + texto), cartão de voto mínimo (título + botão).

3. **Organismos** — **blocos completos** da interface: cabeçalho do player, lista da fila com scroll, painel de regras resumidas. Costumam juntar moléculas e átomos e já conversam com estado ou dados de um subdomínio da tela.
